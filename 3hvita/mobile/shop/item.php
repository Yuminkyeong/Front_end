...
//비타민 상품페이지 따로 연결
$vita_skin_dir = '/item.form.skin.php';
if($it['ca_id'] == '10'){
    $vita_skin_dir = '/vita.item.form.skin.php';
}
...
if($it['it_skin']) {
    ...
    if(is_dir($skin_dir)) {
        $form_skin_file = $skin_dir.$vita_skin_dir;
...
<?php } ?>
    ...
    <div id="sit">
        <?php
        // 상품 구입폼
        include_once($skin_dir.$vita_skin_dir);
        ?>

        <?php
        // 상품 상세정보
        $info_skin = $skin_dir.$vita_skin_dir;
        if(!is_file($info_skin))
            $info_skin = G5_SHOP_SKIN_PATH.$vita_skin_dir;
        include $info_skin;
        ?>
