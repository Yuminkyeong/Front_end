...
// 서브메뉴 OPEN CLOSE
function openNav() {
    document.querySelector("#mySidenav").classList.add("nav_active");
    // document.getElementById("mySidenav").style.width = "30%";
}
function closeNav() {
    document.querySelector("#mySidenav").classList.remove("nav_active");
...
$(function(){
    ...
    if(!isMobile) {
        $slideWrap.css('width',648*6);
        $slideWrap.css('margin-left',-648)
    }
    else{
        $slideWrap.css('width',width*6);
        $slideWrap.css('margin-left',width*-1)
    }
    let $slideView = $('.right-box .slide-view');
...
let $openBtn  = $('.slick-center .openPop');  
let $closeBtn = $('.ingredient-layer .closePop');


$openBtn.on({
    click:function(){
        $('.ingredient-layer').fadeIn(200);
